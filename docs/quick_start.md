...
| Path | Purpose |
|------|---------|
| `components/<component>/cmd/<component>/` | `main` package for each on-board component binary |
| `components/<component>/src/` | Go package code (shared crates like `bus`, `config`, `component` live under their own `components/<name>/src/`) |
| `systems/deliverydron/` | Docker Compose for the full system; images build from `components/<component>/docker/` |
