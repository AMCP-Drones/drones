...
1. К критичным компонентам допускаются только аутентичные и авторизованные сообщения по политике доступа.
2. Критичные межкомпонентные операции выполняются только через доверенный посредник (PEP).
3. Неавторизованные/некорректно адресованные/запрещённые политикой сообщения отклоняются по fail-safe default.
4. Решения контроля доступа (allow/deny, переходы isolation) фиксируются в журнале безопасности.
5. Исполнительные компоненты `navigation`, `motors`, `cargo` входят в доверенный контур и исполняют только разрешённые операции, прошедшие policy-gate.
