...
Responses (when using request/response) follow the shared response shape from `systems/deliverydron/sdk/src` (`success`, `payload`, `correlation_id`, etc.).
...
**Binary:** `systems/deliverydron/delivery_drone/cmd/delivery_drone`  
...
Broker actions (non-exhaustive): `ping`, `get_status`, `echo`, `deliver_package`, `get_delivery_status` — see `systems/deliverydron/delivery/src`.
...
| ATM / port flow on `START` | Not implemented in `systems/deliverydron/autopilot/src` (extension point) |